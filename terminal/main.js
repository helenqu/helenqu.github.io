	...
	switch (command_parsed[0]) {
		case "cd":
			if (command_parsed.length == 1) {
				$("#output").append("<p>" + $(".command:last").val() + " usage: cd <section-name></p>");
				append_input();
				return false;
			}
		...
		case "ls":
			if (current_dir === "~") {
				...
				$("#output").append("<p>links &nbsp; &nbsp; &nbsp; &nbsp;contact</p>");
...
function process_cd(directory) {
	if (current_dir === "~") {
		switch (directory) {
			...
			case "links":
				current_dir = "links";
				$("#output").append("<p><a href='https://scholar.google.com/citations?user=FWPJDb4AAAAJ&hl=en'>google scholar</a> | <a href='http://github.com/helenqu'>github</a> | <a href='http://linkedin.com/in/helen-qu-8b9342a4'>linkedin</a></p>");
				append_input();
				return false;
			case "contact":
				current_dir = "contact";
				$("#output").append("<p>hqu [at] flatironinstitute [dot] org</p>");
				append_input();
				return false;
			default: 
...
function safeFocus() {
	...
	// Don't steal focus if already in an input
	if (document.activeElement === el) return;

	el.focus();
	// Optional: select existing text for terminal feel
